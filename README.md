...
4. **Paginate** — first, elements that belong together are merged into single blocks: anything sharing an Excalidraw group, an arrow/line bound to the elements it connects, and a short standalone text (a heading) glued to whatever follows it whenever it isn't clearly further from that than from what precedes it. Then the content is walked in physical-page-height increments; a block is never cut: if the nominal page height would land inside one, the break backs off to just before it instead (it moves whole to the next page, shrinking this one). The only exception is a single block taller than a full page, which gets a page of its own — rendered at a reduced scale so it still fits within the printable area.
